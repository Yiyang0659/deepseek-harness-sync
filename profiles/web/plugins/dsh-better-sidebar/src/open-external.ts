...
export function revealCommand(path: string, platform: NodeJS.Platform = process.platform): ExternalCommand {
  switch (platform) {
    ...
    case 'win32': {
      // Foreground activation is done by scripts/win-reveal.ps1 (-File =>
      // paths arrive as real argv values, no quoting/escaping corruption).
      const script = fileURLToPath(new URL('../scripts/win-reveal.ps1', import.meta.url))
      return {
        command: 'powershell.exe',
        args: ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', script, '-Target', path],
      }
    }
    default: {
